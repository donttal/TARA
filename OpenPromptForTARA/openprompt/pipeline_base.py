...
class PromptModel(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        # 输出中间层，目前支持BERT和Roberta
        outputs, outputs_mid = self.plm(**input_batch, output_hidden_states=True)
        # print(outputs)

        outputs = self.template.post_processing_outputs(outputs)
        # print(outputs)

        return outputs, outputs_mid
...
# TokenSim
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        Returns:
            :obj:`torch.Tensor`: The logits of the label words (obtained by the current verbalizer).
        """
        outputs, outputs_mid = self.prompt_model(batch)
        ...
        if isinstance(outputs_mid, tuple):
            outputs_at_mask_mid = [self.extract_at_mask(output, batch) for output in outputs_mid]
        else:
            outputs_at_mask_mid = self.extract_at_mask(outputs_mid, batch)
        
        # print("outputs_at_mask")
        # print(outputs_at_mask)
        # print(outputs_at_mask.shape)

        label_words_logits = self.verbalizer.process_outputs(outputs_at_mask, batch=batch)
        return label_words_logits, outputs_at_mask_mid
